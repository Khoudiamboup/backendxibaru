...
export class CommentService {
  ...
  // NOUVELLE MÉTHODE : Récupérer les commentaires en attente
  ...
  // NOUVELLE MÉTHODE : Suppression définitive
  ...
  // NOUVELLE MÉTHODE : Compter les commentaires d'un article
  ...
  // NOUVELLE MÉTHODE : Statistiques des commentaires
  async getStats() {
    const total = await this.commentRepository.count();
    const approved = await this.commentRepository.count({ 
      where: { comment_approved: '1' } 
    });
    const pending = await this.commentRepository.count({ 
      where: { comment_approved: '0' } 
    });
    const spam = await this.commentRepository.count({ 
      where: { comment_approved: 'spam' } 
    });
    const trash = await this.commentRepository.count({ 

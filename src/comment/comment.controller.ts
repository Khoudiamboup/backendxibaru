...
export class CommentController {
  ...
  // NOUVELLE ROUTE : Récupérer les commentaires en attente de modération
  @Get('pending')
  async findPending(): Promise<Comment[]> {
    return await this.commentService.findPending();
  }

  // NOUVELLE ROUTE : Statistiques des commentaires
  @Get('stats')
  async getStats() {
    return await this.commentService.getStats();
  }
  ...
  // NOUVELLE ROUTE : Compter les commentaires d'un article
  @Get('article/:articleId/count')
  async countByArticle(@Param('articleId') articleId: number): Promise<{ count: number }> {
    const count = await this.commentService.countByArticle(Number(articleId));
    return { count };
  }
  ...
  // NOUVELLE ROUTE : Suppression définitive d'un commentaire
  @Delete(':id/permanent')
  async delete(@Param('id') id: number): Promise<{ 
    ...
    return { 
      status: 'success',
      message: `Commentaire ${id} supprimé définitivement` 
    };
  }
}